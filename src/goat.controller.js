...
const example1 = () => {
    if (!validate.validar_campo(['example-1'], 'vacios')) return;
    if (!validate.validar_campo(['example-2'], 'vacios')) return;
    if (!validate.validar_campo(['example-3'], 'numeros')) return;
    if (!validate.validar_campo(['example-4'], 'numeros')) return;
    document.getElementById('form').submit();
}
const example_login = () => {
    if (!validate.validar_campo(['example-email'], 'example-email')) return;
    if (!validate.validar_campo(['example-password'], 'example-password')) return;
    document.getElementById('form').submit();
}
